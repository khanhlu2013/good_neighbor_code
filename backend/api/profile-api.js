...
route.post('/createInvitation',authCheck,async (req,res)=>{
  const {user} = req;
  ...
  try{
    const connection = await new Connection({from : user.id,to : userIdToAdd,})
    .save();
    return res.send(connection);
  }catch(e){
    console.log(e);res.status(500).send()
  }
});

route.post('/modifyInvitation',authCheck,async (req,res)=>{
  const {user} = req;
...
route.get('/friends',authCheck,(req,res)=>{
  const {user} = req;
  (async()=>{
    const friends = await Connection.find({$or:[
      {from:user},
      {to:user}
    ]})
    .populate('from')
