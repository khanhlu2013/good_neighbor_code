...
ConnectionSchema.pre('save', async function(){
  ...
  const reverseConnection = await this.constructor.findOne({from : connection.to,to:connection.from});
  if(reverseConnection){
    throw Error('Error: reverse connection exist');
  }
})
ConnectionSchema.statics.findConnection = async function(user1,user2){
  const Connection = this;
  return await Connection.findOne({$or:[
    {from:user1,to:user2},
    {from:user2,to:user1}
  ]})
}
